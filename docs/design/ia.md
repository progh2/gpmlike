...
- M5 프로토(#10): 진입은 **`CRISIS` 오버레이 → `SortieLite`** 만 (**Auto** 기본). 종료 후 직전 beat 복귀. Hub `Bay`는 **장비/로드아웃만** (전투 시작 아님). 유닛 ID는 `Shoreframe` `WallScout` `Seamkin`(dummy 1종) — [`original-pitch.md`](original-pitch.md) §8. Action Code 없음.
